...
Dado(/^um parser de numero Romano para Arabico$/) do
  @parser = Parser.new
end

Quando(/^eu converto o valor romano "(.*?)"$/) do |valor_romano|
  @valor_convertido = @parser.parser(valor_romano)
end

Entao(/^o numero arabico convertido sera "(.*?)"$/) do |valor_arabico|
  @valor_convertido.should == valor_arabico
end